...
async def lifespan(app: FastAPI):
    ...
    # Load BERT
    try:
        model_path = os.path.join(BASE_DIR, "models", "bert_sentiment")
...
def get_bert_top_words(text, tokenizer, model, top_n=5):
    ...
    if not words: words = text.split()[:10]
    if not words: return []
...
async def predict_tfidf(request: PredictRequest):
    ...
    if not cleaned_text: return {"label": "Neutral", "confidence": 0, "top_words": []}
    ...
    label = "Neutral" if confidence < 0.6 else ("Positive" if label_idx == 1 else "Negative")
    
    return {"label": label, "confidence": confidence, "top_words": get_tfidf_top_words(cleaned_text, models["tfidf_vec"], models["lr"])}
